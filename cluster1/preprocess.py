...
def makeDataArray(artist, song):
    ...
    if songTrace.shape[1] > 7:
        suddenPoint = utils.findSuddenPoint(songTrace)
    else:
        suddenPoint = 0
    songTrace = songTrace[:, suddenPoint:]
    songTrace = np.delete(songTrace, [2, 3], axis=0)
    # songEA = song.getCumulateEA()
    songPercent = song.getPercentInSongs()
    songPercent = songPercent[:, suddenPoint:]
    songPercent = np.delete(songPercent, [2, 3], axis=0)
    # artistEA = artist.getTotalCumulateEA()[song.getEmptyDays():]
    artistPercent = artist.getPercentInArtists()[:, song.getEmptyDays():]
    artistPercent = artistPercent[:, suddenPoint:]
    artistPercent = np.delete(artistPercent, [2, 3], axis=0)
