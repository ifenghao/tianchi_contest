...
def train(artist, embedDim, interval):
    ...
    for index, songList in clusterDict.items():
        ...
        XTrainCluster, yTrainCluster = foldTrain(tracelist, embedDim, interval, 1)
        ...
        for i in range(9):
            for _ in range(7):
                XTrainCluster, yTrainCluster = foldTrain(tracelist, embedDim, interval, 1)
                XPredictCluster = foldPredict(tracelist, embedDim, interval, 1)
...
def shortSongsPredict(shortSongsList, embedDim, interval):
    ...
    for song in shortSongsList:
        array, mean, var = pp.fprocess(song)
        yPredict = itererfModel.train(array, embedDim, interval)
